...
from utils.PINNs.model.NNModel import NNModel
...
class TurbulenceModelPINN(L.LightningModule):
    ...
    Attributes:
        ...
        output_dim (int): The number of outputs from the model.
        hidden_depth (int): The number of hidden layers in the model.
        activation (str): The activation function used in hidden layers.
        loss_phys_momentum_weight (float): The weight for the momentum loss term (physical loss).
        loss_phys_k_weight (float): The weight for the k loss term (physical loss).
        loss_bound_U_weight (float): The weight for the U loss term (boundary loss).
        loss_bound_dUdy_weight (float): The weight for the dUdy loss term (boundary loss).
        loss_bound_P_weight (float): The weight for the P loss term (boundary loss).
        loss_bound_k_weight (float): The weight for the k loss term (boundary loss).
        loss_bound_stress_weight (float): The weight for the stress loss term (boundary loss).
        rho (float): The fluid density.
        model (NNModel): The neural network model for the PINN.
    """
    ...
    def training_step(self, batch, batch_idx):
        """
        Computes the training loss for a single batch. The loss is a combination of the physics-informed loss and data loss.
        ...
        loss_pde = self.physics_informed_loss(predictions, x) # Compute the physics-informed loss
        loss_data = self.data_loss(predictions, targets) # Compute the data loss
    ...
    def configure_optimizers(self):
        ...
        return {
            ...
            "lr_scheduler": {
                "scheduler": scheduler,
                "interval": "epoch",
                "frequency": 100, # Adjust learning rate every 100 epochs
                "monitor": "val_mse", # Monitor the validation MSE
            },
            "monitor": "val_mse",
        }
